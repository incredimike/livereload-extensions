          ...
          return this.beforeEnablingFirst((function(_this) {
            return function(err) {
              ...
              if (err) {
                switch (err) {
                  case 'cannot-connect':
                    console.log('Cannot coneeeeeect');
                    return state.alert(CannotConnectAlert);
                  case 'cannot-download':
                    console.log('Cannot downloooad');
...
module.exports = ExtVersion = '20.1.0';
...
TabState.prototype.send = function(message, data) {
  ...
  console.log('sendMessage', message, this.tab);
  return browser.tabs.sendMessage(this.tab, [message, data]);
};

TabState.prototype.bundledScriptURI = function() {
  return browser.runtime.getURL('livereload.js');
};
